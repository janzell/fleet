...
const GarageDrawer = props => {
  ...
  const handleSave = (e) => {
    ...
    validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        openNotificationWithIcon('success', 'Success', `Garage was successfully ${action}.`);
        resetFields();
        onCancel();
      }
    });
  ...
  const mutateGarage = async (mutation, variables) => {
    await client.mutate({mutation, variables, refetchQueries: [{query: GET_GARAGE_LIST, variables: listOptions}]});
  };
